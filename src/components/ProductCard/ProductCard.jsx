...
const ProductCard = () => {
  ...
  useEffect(() => {
    if (goods) {
      // Remove items listed by the logged-in user
  ...
  const handlePriceChange = (event) => setPriceRange([0, Number(event.target.value)]);
  ...
  const filteredAndSortedGoods = useSelector((state) => state.good.good)
    ?.filter((good) => {
      const categoryMatch =
        activeCategory === "All Products" || good.Category?.category_name === activeCategory;
  ...
  if (isLoading) return <div className="container mx-auto mt-8 mb-8 py-4 px-9">Loading...</div>;

  return (
    <div className="w-full">
      <div className="container mx-auto mt-10 ml-4 mr-4 py-4 px-6 w-full max-w-screen-2xl">
        <h2 className="text-3xl font-semibold mb-6">Featured Goods</h2>
        <div className="flex items-center gap-6 mb-6 pb-2">
          {["All Products", "Technology", "Computers", "Music", "Accessories"].map((category) => (
            <span
              key={category}
      ...
      <div className="flex w-full" data-aos="zoom-in">
        {/* Sidebar Filters */}
        <div className="bg-gray-100 p-5 rounded-lg shadow-sm w-[250px] ml-10 mb-10 flex-shrink-0 mr-5 h-[1000px]">
          {/* Price Range Filter */}
        ...
        {/* Product List */}
        <div className="bg-gray-100 flex flex-wrap gap-5 flex-grow rounded-lg p-5 mr-10 mb-10">
          {filteredAndSortedGoods.length > 0 ? (
            filteredAndSortedGoods.map((product) => (
              ...
              >
                ...
                <h4 className="text-lg font-bold my-2.5">{product.good_name}</h4>
