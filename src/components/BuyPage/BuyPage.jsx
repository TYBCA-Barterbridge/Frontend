...
import { FaRupeeSign, FaTimes, FaArrowLeft, FaTruck, FaShieldAlt } from 'react-icons/fa';
  ...
  const handlePurchase = async () => {
    try {
      ...
      if (itemType === 'good') {
        ...
        response = await buySkill(purchaseData).unwrap();
      }
      ...
      setErrorMessage(err.data?.message || 'Failed to complete purchase. Please try again.');
      setShowConfirmModal(false);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center p-6 bg-white rounded-lg shadow-lg">
          <h2 className="text-xl font-semibold text-gray-800">No item selected for purchase</h2>
          <button
            onClick={() => navigate('/')}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center justify-center mb-6">
              <img
                src={item[`${itemType === 'good' ? 'Good' : 'Skill'}_imgs`]?.[0]?.img_url || '/placeholder.svg'}
                alt={item[`${itemType}_name`]}
                className="w-40 h-40 object-cover rounded-lg"
              />
            </div>
            <h2 className="text-2xl font-bold text-center mb-6">{item[`${itemType}_name`]}</h2>
              ...
              <div className="space-y-4">
                <div className="flex items-center">
                  ...
                  <div className="ml-4 flex-1">
                    <h4 className="text-sm font-medium text-gray-900">{item[`${itemType}_name`]}</h4>
                    <p className="mt-1 text-sm text-gray-500">{itemType.charAt(0).toUpperCase() + itemType.slice(1)}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-medium text-gray-900">₹{item[`${itemType}_amount`]}</p>
                  </div>
                </div>
                ...
                <div className="border-t border-gray-200 pt-4 space-y-3">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium">₹{item[`${itemType}_amount`]}</span>
                  </div>
                  <div className="flex justify-between text-sm">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full">
            ...
            <p className="text-gray-600 mb-4">
              ...
              <span className="font-semibold">₹{item[`${itemType}_amount`]}</span>?
            </p>
